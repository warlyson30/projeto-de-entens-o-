...
router.get('/:id',authenticateToken, (req,res) => {
    const lembrete = lembretes.find((l) => l.id === parseInt(req.params.id) && l.userId === req.user.id);
...
router.post('/', authenticateToken, (req,res) => {
    ...
    if (!titulo) {
        return res.status(404).json({erro: "Título Obrigatório"});
    }
...
router.put('/:id', authenticateToken, (req,res) => {
    const index = lembretes.findIndex((l) => l.id === parseInt(req.params.id) && l.userId === req.user.id);
    ...
    lembretes[index] = {
        ...lembretes[index],            titulo: titulo || lembretes[index].titulo,
        descricao: descricao !== undefinded ? descricao: lembretes[index].descricao,
        ...
        materiaId: materiaId !== undefindes ? materiaId : lembretes[index].materiaId,
        atualizadoEm: new Date().toISOString(),
        };
...
router.delete('/:id', authenticateToken, (req,res) => {
    ...
    return res.json({ messagem: "Lembrete Removido com Sucesso"});
});
