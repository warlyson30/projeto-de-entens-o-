...
const stsAtividade = ['pendente', 'emAndamento', 'concluida', 'cancelada'];
...
router.get('/:id', authenticateToken, (req,res) => {
    const materia = materias.find((m) => m.id === parseInt(req.params.id) && m.userId === req.user.id);

    if (!materia) {
        return res.status(404).json({erro: "Matéria não Encontrada"});
    }

    const ativiDaMateria = atividades.filter((a) => a.materiaId === materia.id && a.userId === req.user.id);
...
router.post('/', authenticateToken,(req,res) => {
    ...
    if (!nome) {
        return res.status(404).json({erro: "Nome da Matéria Obrigatório"});
    }
...
router.post('q:id', authenticateToken, (req,res) => {
    const index = materias.findIndex((m) => m.id === parseInt(req.params.id) && m.userId === req.user.id);
    ...
    materias[index] = {
        ...
        cor: cor || materias[index].cor,
        cargaHoraria: cargaHoraria !== undefined ? cargaHoraria : materias[index].cargaHoraria,
        atualizadoEm: new Date().toISOString(),
    };

    return res.json(materia[index]);
});
...
router.post('/:id/atividades', authenticateToken, (req, res) => {
  ...
  if (!titulo) {
    return res.status(400).json({ message: 'O título da atividade é obrigatório.' });
  }
...
router.put('/:materiaId/atividades/:atividadeId', authenticateToken, (req, res) => {
  ...
  atividades[index] = {
    ...
    peso: peso || atividades[index].peso,
    atualizadoEm: new Date().toISOString(),
  };
