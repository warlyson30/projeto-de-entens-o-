...
router.post('/', authenticateToken, (req, res) => {
  ...
  if (isNaN(Date.parse(dataInicio))) {
    return res.status(400).json({ mensagem: 'dataInicio inválida. Use o formato ISO 8601 (YYYY-MM-DD).' });
  }

  if (dataFim && isNaN(Date.parse(dataFim))) {
    return res.status(400).json({ mensagem: 'dataFim inválida. Use o formato ISO 8601 (YYYY-MM-DD).' });
  }
